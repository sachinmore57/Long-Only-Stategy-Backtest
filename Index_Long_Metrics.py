...
data_dir = 'C:/Users/Sachin M/PycharmProjects/IB_Projects/Trade1'

tradelog_df = pd.read_csv('BN_Daily_Tradelog_rev2.csv', index_col=[0], parse_dates=True)

def backtest_metrics(tradelog_df, capital, quantity):
    try:
        tradelog_df['Entry_Date'] = tradelog_df['Entry_Date'].apply(lambda x: datetime.datetime.strptime(x, '%Y-%m-%d'))
        initial_capital = capital
        tradelog_df['Quantity'] = quantity
        tradelog_df['Total_PnL'] = tradelog_df['PnL'] * 25

        tradelog_df['Pnl_Cumsum'] = tradelog_df['Total_PnL'].cumsum()
        ...
        for i in range(len(tradelog_df)):
            if i == 0:
                tradelog_df['Equity'].iloc[i] = initial_capital + tradelog_df['Total_PnL'].iloc[i]
                tradelog_df['Rate_of_return'].iloc[i] = (tradelog_df['Total_PnL'].iloc[i] / initial_capital) * 100
            else:
                tradelog_df['Equity'].iloc[i] = tradelog_df['Equity'].iloc[i - 1] + tradelog_df['Total_PnL'].iloc[i]
                tradelog_df['Rate_of_return'].iloc[i] = (tradelog_df['Total_PnL'].iloc[i] / initial_capital) * 100

        ## Win Rate
        win_rate = round(len(tradelog_df[tradelog_df['Total_PnL'] > 0]) / len(tradelog_df) * 100, 2)

        ## Mean Win
        mean_win = round(tradelog_df[tradelog_df['Total_PnL'] > 0]['Total_PnL'].mean(), 2)

        ## Mean Loss
        mean_loss = round(tradelog_df[tradelog_df['Total_PnL'] < 0]['Total_PnL'].mean(), 2)
        ...
        ##Max Drawdown
        tradelog_df['Drawdown'] = tradelog_df['Pnl_Cumsum'] - tradelog_df['Pnl_Cumsum'].cummax()
        ...
        trade_log_equity_high['number_days_between_equity_highs'] = (trade_log_equity_high['Entry_Date'] -
                                                                     trade_log_equity_high['Entry_Date'].shift())
        recovery_days = int(trade_log_equity_high['number_days_between_equity_highs'].apply(lambda x: x.days).max())

        ##Cagr
        backtest_days = (tradelog_df.iloc[-1]['Entry_Date'].date() - tradelog_df.iloc[0]['Entry_Date'].date()).days
        ...
        start_date = tradelog_df.iloc[0]['Entry_Date'].date()
        end_date = tradelog_df.iloc[-1]['Entry_Date'].date()
        number_of_trades = len(tradelog_df)

        ##number of wins
        number_of_wins = len(tradelog_df[tradelog_df['Total_PnL'] > 0])

        ##number of losses
        number_of_losses = len(tradelog_df[tradelog_df['Total_PnL'] < 0])

        ## average profit per trade
        average_profit_per_trade = round(tradelog_df[tradelog_df['Total_PnL'] > 0]['Total_PnL'].mean(), 2)

        ## average loss per trade
        average_loss_per_trade = round(tradelog_df[tradelog_df['Total_PnL'] < 0]['Total_PnL'].mean(), 2)

        ##max pnl
        max_pnl = round(tradelog_df['Total_PnL'].max(), 2)

        ##min_pnl
        min_pnl = round(tradelog_df['Total_PnL'].min(), 2)

        ##median_of_trade
        median_of_trade = round(tradelog_df['Total_PnL'].median(), 2)

        ##Profit factor
        gross_profit = tradelog_df[tradelog_df['Total_PnL'] > 0]['Total_PnL'].sum()
        gross_loss = tradelog_df[tradelog_df['Total_PnL'] < 0]['Total_PnL'].sum()
        ...
        for i in range(1, len(tradelog_df)):
            if tradelog_df['Total_PnL'].iloc[i - 1] > 0:
